...
class BackendController extends YBackController
{
    ...
    public function actionAjaxFileUpload()
    {
        if (!empty($_FILES['file']['name']))
        {
            ...
            if ($image)
            {
                ...
                Yii::app()->ajax->rawText(CHtml::image('/uploads/' . date('dmY') . '/'. $newFileName));
            }
        }
    ...
    // Recursive removes dir and its contents
    protected function rrmdir($rrdir) {
        foreach(glob($rrdir . '/*') as $file) {
            if(is_dir($file))
                rrmdir($file);
            else
                unlink($file);
    ...
    {
        // flush timthumb cache too
        $this->rrmdir($_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'cache');
